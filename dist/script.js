...
const candleCount = 6; // Number of candles
...
// make renderer background transparent so the fireworks canvas shows through
renderer.setClearColor(0x000000, 0);
// assign a class so CSS can place it above the fireworks canvas
...
function flame() {
	...
	// ensure flame renders above candle geometry
...
// react to fireworks explosions by adding temporary colored lights
document.addEventListener('firework-explosion', (e) => {
	try{
		const { x, y, color, intensity } = e.detail;
		const canvas = document.getElementById('fireworks');
		if (!canvas) return;
		const rectW = canvas.width || window.innerWidth;
		const rectH = canvas.height || window.innerHeight;
		// convert canvas pixel coords to NDC (-1..1)
		const ndcX = (x / rectW) * 2 - 1;
		const ndcY = - (y / rectH) * 2 + 1;
		const vec = new THREE.Vector3(ndcX, ndcY, 0.5).unproject(camera);

		// main burst light
		const burst = new THREE.PointLight(color, 2.0 * intensity, 14 + 40 * intensity, 2);
		burst.position.copy(vec);
		scene.add(burst);

		// gentle fill light to color the cake/table
		const fill = new THREE.PointLight(color, 0.8 * intensity, 30 + 60 * intensity, 2);
		// position fill a bit lower (towards cake)
		const fillPos = vec.clone();
		fillPos.y = Math.max(0.5, fillPos.y - 2.0);
		fill.position.copy(fillPos);
		scene.add(fill);

		// fade lights over time
		let t = 0;
		const fadeInterval = setInterval(() => {
			t += 0.04;
			burst.intensity = Math.max(0, (1 - t) * 2.0 * intensity);
			fill.intensity = Math.max(0, (1 - t) * 0.8 * intensity);
			if (t >= 1) {
...
// When audio starts playing, after 15s enable blowing option (but keep music playing)
audio.addEventListener('play', function() {
	if (blowTimer) clearTimeout(blowTimer);
	blowTimer = setTimeout(() => {
		allowBlowout = true;
		holdReminder.style.display = 'flex';
		setTimeout(function() { holdReminder.classList.add('show'); }, 10);
	}, 15000);
});
...
function showCongratulation() {
  ...
  overlay.style.opacity = '1';
	// stop and hide fireworks
	if (window.stopFireworks) window.stopFireworks();
	// hide Three.js canvas so only the overlay text is visible
	try{
		if(renderer && renderer.domElement) renderer.domElement.style.display = 'none';
	}catch(e){}
	// hide UI overlays
	const vol = document.getElementById('volume-reminder');
	if(vol) vol.style.display = 'none';
	const hold = document.getElementById('hold-reminder');
...
function blowOutCandles() {
	(async () => {
		...
		candles.children.forEach(candle => {
			...
			promises.push(extinguishCandle(candle, speed));
		});

		// wait until all candles have finished extinguishing
		await Promise.all(promises);

		// after candles are fully out, gradually increase ambient light then show congratulation
